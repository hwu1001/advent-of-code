...
pub fn main() !void {
    ...
    var sleepiest_min_index: usize = 0;
    for (sleepiest_guard.?.value.minutes[0..]) |min, ndx| {
        if (min > sleepiest_minute) {
            sleepiest_minute = min;
            sleepiest_min_index = ndx;
        }
    }
    while (iter.next()) |entry| {
        for (entry.value.minutes[0..]) |min, ndx| {
            if (min > sleepiest_minute) {
                sleepiest_minute = min;
                sleepiest_min_index = ndx;
    ...
    std.debug.warn("Id {} Min {} res {}\n", sleepiest_guard.?.key, sleepiest_min_index, sleepiest_guard.?.key * sleepiest_min_index);
}
        ...
        switch(char) {
            ...
            'G', 'f', 'w' => {
                if (state == State.Action and index < 1) {
                    // buf[0] = char; this didn't work ¯\_(ツ)_/¯
                    switch(char) {
