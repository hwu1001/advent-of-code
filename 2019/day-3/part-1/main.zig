...
pub fn main() !void {
    ...
    var pt_set = std.AutoHashMap(Point, void).init(std.heap.page_allocator);
    defer pt_set.deinit();
    var it = wire_one_pts.iterator();
    while (it.next()) |val| {
        if (wire_two_pts.contains(val.key)) {
            _ = try pt_set.put(val.key, {});
        }
    }
    ...
    while (it2.next()) |kv| {
        var pt = kv.key;
        var d = calcManhattanDist(pt.x, pt.y, 0, 0);
        if (start) {
            start = false;
            lowest_val = d;
        }
...
fn getAllPoints(allocator: *std.mem.Allocator, wire: *std.ArrayList(std.ArrayList(u8))) !std.AutoHashMap(Point, void) {
